...
func (r *BhagyaTestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			log.Info("BhagyaTest resource not found. Ignoring since object must be deleted")
			return ctrl.Result{}, nil
		}
		// Error reading the object - requeue the request.
		log.Error(err, "Failed to get BhagyaTest")
		return ctrl.Result{}, err
	}

	// Check if the deployment already exists, if not create a new one
	found := &appsv1.Deployment{}
	err = r.Get(ctx, types.NamespacedName{Name: bhagya.Name, Namespace: bhagya.Namespace}, found)
	if err != nil && errors.IsNotFound(err) {
		// Define a new deployment
		dep := r.deploymentForBhagyaTest(bhagya)
	...
	if !reflect.DeepEqual(podNames, bhagya.Status.Nodes) {
		bhagya.Status.Nodes = podNames
		err := r.Status().Update(ctx, bhagya)
		if err != nil {
			log.Error(err, "Failed to update BhagyaTest status")
			return ctrl.Result{}, err
		}
	}
...
func (r *BhagyaTestReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&cachev1.BhagyaTest{}).
